...
from models import Flow, Refine, DynFilter
import tools
import argparse
import yaml
import time
...
class DynFilterModel(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ct0, ct1, self.segs, index_paths, self.cts, self.all_cts, self.all_segs = batch
        self.marked = [_[0] for _ in index_paths]
        self.Flow.eval()
        with torch.no_grad():
            # forward
    ...
    def on_test_end(self):
        self.segs = [_[0] for _ in self.segs]
        self.cts  = [_[0] for _ in self.cts] 
        ...
        for i in range(len(self.forward_flows)+1):
            if i in self.marked:
                segs[i] = self.segs[j]
                cts[i] = self.cts[j] 
                gen_f_cts[i] = self.cts[j] 
                gen_b_cts[i] = self.cts[j] 
        ...
        for idx in range(len(self.forward_flows)):
            if idx in self.marked:
                f_ct = cts[idx].unsqueeze(0).unsqueeze(0) 
                f_seg = segs[idx].unsqueeze(0)
            else:
                # forward
                f_ct = tools.warp(f_ct, self.forward_flows[idx-1]) 
                f_seg = tools.warp_seg(f_seg, self.forward_flows[idx-1])
                segs[idx] = [f_seg]
                cts[idx] = [f_ct] 

                gen_f_ct = tools.warp(self.all_cts[idx-1].unsqueeze(0), self.forward_flows[idx-1]) 
                gen_f_cts[idx] = gen_f_ct[0, 0] 
        ...
        for idx in range(len(self.backward_flows), 0, -1):
            if idx in self.marked:
                b_ct = cts[idx].unsqueeze(0).unsqueeze(0) 
                b_seg = segs[idx].unsqueeze(0)
            else:
                # backward
                b_ct = tools.warp(b_ct, self.backward_flows[idx]) 
                b_seg = tools.warp_seg(b_seg, self.backward_flows[idx])
                segs[idx].append(b_seg)
                cts[idx].append(b_ct) 

                gen_b_ct = tools.warp(self.all_cts[idx+1].unsqueeze(0), self.backward_flows[idx]) 
                gen_b_cts[idx] = gen_b_ct[0, 0] 
        ...
        ct_forward_save_folder = os.path.join(self.cfg['save_folder'], 'ct_forward') 
        ct_backward_save_folder = os.path.join(self.cfg['save_folder'], 'ct_backward') 
        gen_f_ct_save_folder = os.path.join(self.cfg['save_folder'], 'gen_f_ct') 
        gen_b_ct_save_folder = os.path.join(self.cfg['save_folder'], 'gen_b_ct') 
        ...
        os.makedirs(gen_f_ct_save_folder) 
        os.makedirs(gen_b_ct_save_folder) 
        os.makedirs(forward_save_folder)
        os.makedirs(backward_save_folder)
        os.makedirs(union_save_folder)
        ...
        os.makedirs(ct_forward_save_folder) 
        os.makedirs(ct_backward_save_folder) 
        ...
        for i in range(len(gen_f_cts)):
            np.save(os.path.join(gen_f_ct_save_folder, 'ct{:03d}*.npy'.format(i)), gen_f_cts[i].cpu().numpy()) 
            np.save(os.path.join(gen_b_ct_save_folder, 'ct{:03d}*.npy'.format(i)), gen_b_cts[i].cpu().numpy()) 
